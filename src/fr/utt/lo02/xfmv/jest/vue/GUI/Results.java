...
public class Results extends JPanel {
    ...
    private void initComponents() {
        ...
        //---- player1Label ----
        player1Label.setText(Game.getInstance().getPlayers().get(0).toString() + " - " + Game.getInstance().getPlayers().get(0).getScore() + " Points");
        player1Label.setFont(new Font("Ubuntu Light", Font.BOLD, 18));
        add(player1Label, "cell 0 1 5 1,alignx center,growx 0");

        //---- label1 ----
        label1.setText(Game.getInstance().getPlayers().get(0).getJest().toString());
        add(label1, "cell 2 2,alignx center,growx 0");

        //---- player2Label ----
        player2Label.setText(Game.getInstance().getPlayers().get(1).toString() + " - " + Game.getInstance().getPlayers().get(1).getScore() + " Points");
        player2Label.setFont(new Font("Ubuntu Light", Font.BOLD, 18));
        add(player2Label, "cell 0 3 5 1,alignx center,growx 0");

        //---- label2 ----
        label2.setText(Game.getInstance().getPlayers().get(1).getJest().toString());
        add(label2, "cell 2 4,alignx center,growx 0");

        //---- playe3Label ----
        playe3Label.setText(Game.getInstance().getPlayers().get(2).toString() + " - " + Game.getInstance().getPlayers().get(2).getScore() + " Points");
        playe3Label.setFont(new Font("Ubuntu Light", Font.BOLD, 18));
        add(playe3Label, "cell 0 5 5 1,alignx center,growx 0");

        //---- label3 ----
        label3.setText(Game.getInstance().getPlayers().get(2).getJest().toString());
        add(label3, "cell 2 6,alignx center,growx 0");

        if (Game.getInstance().getPlayerCount() == 4) {
            //---- player4Label ----
            player4Label.setText(Game.getInstance().getPlayers().get(3).toString() + " - " + Game.getInstance().getPlayers().get(3).getScore() + " Points");
            player4Label.setFont(new Font("Ubuntu Light", Font.BOLD, 18));
            add(player4Label, "cell 2 7,alignx center,growx 0");

            //---- label4 ----
            label4.setText(Game.getInstance().getPlayers().get(3).getJest().toString());
            add(label4, "cell 2 8,alignx center,growx 0");
        }


        //---- winnerLabel ----
        winnerLabel.setText(Game.getInstance().getGameWinner().toString() + " a remporté la partie !");
